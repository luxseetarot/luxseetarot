/**
 * Lot 8: pin 841–1240 — stile master + 5 nicchie.
 *   node scripts/generate-pinterest-lot8.mjs
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import sharp from 'sharp';
import { ADVICE } from './lot8-advice-data.mjs';
...
const NICHE_STYLES = {
  archetipi: [
    { name: 'archetype-hermit', accent: '#d4af6a', accent2: '#f0d78a', foilDeep: '#8a6a30', mute: '#c4a86a', light: false, plate: 'plate-archetype-hermit.png' },
    { name: 'archetype-empress', accent: '#e8b4a0', accent2: '#f6d2c4', foilDeep: '#8a5040', mute: '#c89888', light: false, plate: 'plate-archetype-empress.png' },
    { name: 'violet-mirror', accent: '#c9a8e0', accent2: '#e4d0f4', foilDeep: '#684878', mute: '#9880b0', light: false, plate: 'plate-violet-mirror.png' },
  ],
  tempo: [
    { name: 'time-watch', accent: '#d4af6a', accent2: '#f0d78a', foilDeep: '#8a6a30', mute: '#c4a86a', light: false, plate: 'plate-time-watch.png' },
    { name: 'time-tower', accent: '#c87941', accent2: '#e8a060', foilDeep: '#6a3a1c', mute: '#a87850', light: false, plate: 'plate-time-tower.png' },
    { name: 'black-ring', accent: '#f0c75e', accent2: '#ffe9a0', foilDeep: '#8a6a20', mute: '#d0b060', light: false, plate: 'plate-black-ring.png' },
  ],
  amore_karmico: [
    { name: 'love-hands', accent: '#e8b4a0', accent2: '#f6d2c4', foilDeep: '#8a5040', mute: '#c89888', light: false, plate: 'plate-love-hands.png' },
    { name: 'love-rings', accent: '#d4af6a', accent2: '#f0d78a', foilDeep: '#8a6a30', mute: '#c4a86a', light: false, plate: 'plate-love-rings.png' },
    { name: 'wine-rose', accent: '#e8b4a0', accent2: '#f6d2c4', foilDeep: '#8a5040', mute: '#c89888', light: false, plate: 'plate-wine-heart.png' },
  ],
  cyber: [
    { name: 'cyber-neon', accent: '#e060c0', accent2: '#60e0ff', foilDeep: '#602080', mute: '#a080c0', light: false, plate: 'plate-cyber-neon.png' },
  ...
  cozy: [
    { name: 'cozy-tea', accent: '#6b2d3c', accent2: '#9a4558', foilDeep: '#3d1822', mute: '#7a4a52', light: true, plate: 'plate-cozy-tea.png' },
    { name: 'cream-stairs', accent: '#6b2d3c', accent2: '#9a4558', foilDeep: '#3d1822', mute: '#7a4a52', light: true, plate: 'plate-cream-stairs.png' },
...
function pickStyle(niche, i) {
  const list = NICHE_STYLES[niche] || NICHE_STYLES.cozy;
...
async function main() {
  ...
  console.log(`Lot8: ${pins.length} pin (${FIRST_ID}–${FIRST_ID + 399})`);
  const t0 = Date.now();
  ...
  fs.writeFileSync(path.join(OUT, 'LOT8-NICCHIE.txt'), Object.entries(niches).map(([k, v]) => `${k}: ${v}`).join('\n') + '\n');
