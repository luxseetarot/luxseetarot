/**
 * Upload lot9 pins (1241–1640) to Catbox.
 *   node scripts/upload-lot9-catbox.mjs --all
 */
import fs from 'fs';
...
async function uploadRange(startId, count) {
  ...
  fs.writeFileSync(path.join(PINS, `lot9-batch${batchNum}-urls.json`), JSON.stringify(urls, null, 2));
  fs.writeFileSync(path.join(PINS, `lot9-batch${batchNum}-schedule.json`), JSON.stringify(schedule, null, 2));
  console.log(`Done ${batch.length}. lot9-batch${batchNum}`);
}
