(function () {
  ...
  var css = [
    ...
    '#lux-exit-form{display:grid;grid-template-columns:1fr;gap:10px}',
    ...
    '#lux-exit-err{display:none;color:#ff9b9b;font-size:12px;text-align:center;margin:0;font-family:Inter,system-ui,sans-serif}',
    '#lux-exit-submit{width:100%;display:inline-flex;align-items:center;justify-content:center;margin-top:2px;padding:13px 14px;border:0;border-radius:999px;cursor:pointer;font-family:Cinzel,Georgia,serif;font-size:12px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:#1a1208;background:linear-gradient(135deg,#f0d7a0,#c9a05a 55%,#b8893a);box-shadow:0 8px 28px rgba(201,160,90,.35);transition:transform .2s,box-shadow .2s}',
    '#lux-exit-submit:hover{transform:translateY(-1px);box-shadow:0 12px 34px rgba(201,160,90,.45)}',
    '#lux-exit-note{font-family:Inter,system-ui,sans-serif;font-size:11px;line-height:1.4;text-align:center;color:rgba(154,145,168,.8);margin:2px 0 0}',
    '#lux-exit-note a{color:rgba(232,201,140,.85);text-decoration:none}',
    '@media (max-width:380px){#lux-exit-card{border-radius:18px}#lux-exit-top,#lux-exit-body{padding-left:16px;padding-right:16px}}',
  ...
  function hasHomeForm() {
    return !!(document.getElementById('form') && document.getElementById('name') && document.getElementById('q'));
  }
      ...
      '<div id="lux-exit-card">' +
        ...
        '<div id="lux-exit-body">' +
          '<form id="lux-exit-form" novalidate>' +
            ...
            '<div class="lux-field"><label for="lux-exit-q">La tua domanda</label><textarea id="lux-exit-q" name="q" maxlength="600" rows="2" placeholder="Es. Cosa mi aspetta in amore?" required></textarea></div>' +
  ...
  function onSubmit(e) {
    ...
    if (!name || question.length < 3) {
      err.textContent = 'Inserisci nome e domanda per ricevere la lettura.';
      err.style.display = 'block';
      return;
    }
    err.style.display = 'none';

    var data = { name: name, question: question };
